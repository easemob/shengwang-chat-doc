...
- 新增 `Message#Broadcast` 属性用于判断该消息是否为聊天室全局广播消息。可通过[调用 REST API 发送聊天室全局广播消息](/docs/sdk/server-side/message_chatroom.html#发送聊天室全局广播消息)。
...
- 消息修改回调 `IChatManagerDelegate#OnMessageContentChanged` 中支持返回[通过 RESTful API 修改的自定义消息](/docs/sdk/server-side/message_modify_text_custom.html)。
