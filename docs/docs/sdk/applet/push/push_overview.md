...
**声网 IM Applet SDK 本身不支持离线推送，只支持对移动端离线推送进行如下配置**：
...
其中，**推送高级功能包括设置推送通知方式、免打扰模式和自定义推送模板**。使用前，你需要在 [声网即时通讯控制台](https://console.easemob.com/user/login)的**即时通讯 > 功能配置 > 功能配置总览**页面激活。高级功能开启后，如需关闭必须联系商务，因为该操作会删除所有相关配置。
