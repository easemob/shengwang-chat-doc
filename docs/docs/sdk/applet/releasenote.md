...
- [IM SDK] uni-app SDK 支持 [uni-app 推送插件](/docs/sdk/applet/push/uniapp_push.html)。
...
- [IM SDK] 消息修改回调 `onModifiedMessage` 中支持返回[通过 RESTful API 修改的自定义消息](/docs/sdk/server-side/message_modify_text_custom.html)。
...
- [IM SDK] 消息结构新增 `broadcast` 字段, 用于判断该消息是否为聊天室全局广播消息。可通过[调用 REST API 发送聊天室全局广播消息](/docs/sdk/server-side/message_chatroom.html#发送聊天室全局广播消息)。
...
- [IM SDK] [getJoinedGroups](/docs/sdk/web/group_manage.html#获取群组列表) 方法中增加请求参数支持返回群组成员人数和自己的角色。
...
- [IM SDK] 新增群聊消息限流错误码 [MESSAGE_CURRENT_LIMITING](/docs/sdk/web/error.html)
- [IM SDK] 邀请加入群聊回调 `onGroupChange` 返回中新增 群名称 参数值。

### 优化

- [IM SDK] 支持批量查询群组详情 [getGroupInfo](/docs/sdk/web/group_manage.html#获取群组详情信息)。
...
- [IM SDK] 新增 [getConversationList](/docs/sdk/web/conversation_list.html) 方法解析会话中的最新一条消息；
