...
- **历史消息**：在服务器上的存储时间与你订阅的套餐包有关，详见[产品价格](billing_strategy.html#套餐服务对比)。
...
- 群组总数、单个群的成员数和用户可加入的群组数取决于套餐版本，详见[套餐包功能详情](billing_strategy.html#套餐服务对比)。
...
不同套餐版本支持的聊天室总数，详见[套餐包功能详情](billing_strategy.html#套餐服务对比)。
...
- 单个 App ID 下的每个用户的好友数量上限与套餐包版本相关，详见[套餐包功能详情](billing_strategy.html#套餐服务对比)。
- 好友备注的长度不能超过 100 个字符。
