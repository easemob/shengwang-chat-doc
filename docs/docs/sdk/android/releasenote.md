...
修复调用 [EMPushManager#getSilentModeForConversations](/docs/sdk/android/push/push_notification_mode_dnd.html#获取多个会话的推送通知设置)方法获取会话的免打扰状态失败的问题。
...
- [IM SDK] [发送前回调](/docs/sdk/server-side/callback_presending.html)时修改的[消息扩展字段](/docs/sdk/android/message_send_receive.html#使用消息扩展字段)，会同步到发送方。
...
### [单群聊 UIKit](/docs/uikit/chatuikit/android/chatuikit_overview.html)
...
- [IM SDK] 新增[从服务器拉取离线消息的开始和结束的事件回调](/docs/sdk/android/connection): `EMConnectionListener#onOfflineMessageSyncStart` 和 `EMConnectionListener#onOfflineMessageSyncFinish`。
...
### [单群聊 UIKit](/docs/uikit/chatuikit/android/chatfeature_message.html#消息置顶)
...
- [IM SDK] 支持[会话推送通知方式的本地存储](/docs/sdk/android/push/push_notification_mode_dnd.html#从服务器获取所有会话的推送通知方式设置)。
...
- [EaseIMKIt 文档](/docs/uikit/chatuikit/android/chatuikit_overview.html) 
...
- [IM SDK] 消息修改回调 `com.hyphenate.EMMessageListener#onMessageContentChanged` 中支持返回[通过 RESTful API 修改的自定义消息](/docs/sdk/server-side/message_modify_text_custom.html)。
...
- [IM SDK] 新增 `EMMessage#isBroadcast` 属性用于判断通过该消息是否为聊天室全局广播消息。可通过[调用 REST API 发送聊天室全局广播消息](/docs/sdk/server-side/message_chatroom.html#发送聊天室全局广播消息)。
...
- [IM SDK] 新增[荣耀推送](/docs/sdk/android/push/push_honor.html)。
...
- [IM SDK] [发送前回调](/docs/sdk/server-side/callback_presending.html)：发送失败时返回给 app 用户的错误描述中增加你自定义的错误信息（即 [响应体参数](/docs/sdk/server-side/callback_presending.html#响应-body) code 信息）。
