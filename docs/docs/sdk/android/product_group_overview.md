...
| 漫游消息/历史消息记录存储 | 支持漫游消息和历史记录的存储。服务器存储漫游消息和历史消息记录的时间取决于你订阅的 IM 套餐包，详见[产品价格](billing_strategy.html#套餐服务对比)。<br/> - 你可以[调用 REST API 接口从服务端获取历史消息记录 JSON 文件](/docs/sdk/server-side/message_historical.html)。<br/> - 你还可以[调用客户端接口从服务端获取指定会话的漫游消息](/docs/sdk/android/message_retrieve.html#从服务器获取指定会话的消息)，在多设备间同步。 | 支持漫游消息和历史记录的存储。服务器存储漫游消息和历史消息记录的时间取决于你订阅的 IM 套餐包，详见[产品价格](billing_strategy.html#套餐服务对比)。<br/> - 你可以[调用 REST API 接口从服务端获取历史消息记录 JSON 文件](/docs/sdk/server-side/message_historical.html)。<br/> - 你还可以[调用客户端接口从服务端获取指定会话的漫游消息](/docs/sdk/android/message_retrieve.html#从服务器获取指定会话的消息)，在多设备间同步。<br/> - 如果需要用户新加入聊天室时服务器发送最近的漫游消息，可以联系商务开通，每个会话默认支持 10 条消息，最多可调整至 200 条。|
...
- 群成员的数量根据不同的套餐版本而不同，详见[产品价格](billing_strategy.html#套餐服务对比)。
