...
推送通知方式和免打扰模式为推送的高级功能，默认为开启状态。如需关闭推送高级功能必须联系商务，因为该操作会删除所有相关配置。

![image](/images/ios/push/push_ios_27_enable_push.png)

## 推送通知方式

为优化用户在处理大量推送通知时的体验，声网 IM 在 app 和会话层面提供了推送通知方式和免打扰模式的细粒度选项。
...
你可以调用 `AgoraChatPushManager#syncSilentModeConversationsFromServerCompletion:` 方法从服务器同步所有会话的推送通知方式设置。同步后成功后的结果会存储到本地数据库，然后你可以通过 `AgoraChatConversation#disturbType` 查询当前会话的推送通知方式。

```objectivec
[AgoraChatClient.sharedClient.pushManager syncSilentModeConversationsFromServerCompletion:^(AgoraChatError * _Nullable error) {
...
[AgoraChatClient.sharedClient.pushManager setSilentModeForConversation:@"" conversationType:AgoraChatTypeChat params:param completion:^(AgoraChat * _Nullable aResult, AgoraChatError * _Nullable aError) {
...
// 接收多设备事件回调
@interface ViewController: UIViewController <AgoraChatMultiDevicesDelegate> 
...
你可以调用 `setSilentModeForConversation` 设置指定会话的推送通知，并通过指定 `AgoraChatSilentModeParam` 字段设置推送通知方式和免打扰模式，如以下代码示例所示：
