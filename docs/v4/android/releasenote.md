...
- 将 libwebsockets 升级至 4.5.8，提升网络通信的稳定性和兼容性。
- 优化单聊会话已读回执的内存缓存更新逻辑，无需重新从数据库加载全部缓存消息。
- 优化会话内存缓存清理逻辑，确保底层会话缓存与 Java 层会话对象缓存同步清理。
- 优化消息附件状态变更回调，将回调统一切换至主队列，并增强主队列任务的异常保护。

#### 修复

- 修复并发获取 DNS 配置时，后发调用可能等待超时的问题。
- 修复发送消息后替换消息 ID 时，相同时间戳的多条消息可能出现缓存排序异常的问题。
- 修复附件下载内容不完整时仍可能被判定为成功，以及通过 Content URI 下载时文件大小判断不正确的问题。
- 修复附件上传或下载在失败重试过程中可能先回调 `onError`、随后又回调 `onSuccess` 的问题。
- 修复分片上传时，文件偏移跳转失败或分片长度超出剩余文件大小未被正确识别的问题。
- 修复消息撤回时会话对象不存在可能导致空指针异常的问题。
- 修复部分视频无法生成缩略图时可能导致空指针异常的问题。
...
- 修复退出登录或推送 Token 上传失败后，重新登录时可能无法重新绑定推送 Token 的问题。
- 修复离线消息同步与断开连接流程中可能出现的锁等待问题。
- 修复消息扩展属性中包含格式错误 JSON 数据时可能触发 JNI 异常的问题。
- 修复部分场景下语音文件发送失败的问题。
...
1. 修复 `checkdns` 函数在部分场景下的死锁问题。
2. 修复发送消息前将缩略图设置为空时，导致图片和视频消息发送异常的问题。
...
- 底层 curl HTTP 请求支持 SSL 证书校验。
- 发送消息时，若翻译失败，SDK 返回错误码 1113，提示消息翻译失败（`TRANSLATE_MESSAGE_FAIL`）。

#### 修复

- 修复添加好友时，用户不存在场景下返回错误码不准确的问题。
- 修复群成员变更通知先于 REST 接口返回时导致的群人数不准确问题。
- 修复分片上传文件时 Android 平台层缺少对应实现，以及无法识别 `file://` 格式的本地文件 URI 导致上传失败的问题。
...
修复 4.17.0 引入的特定 WebSocket 服务器配置下无法登录的问题。

## v4.18.0 Dev 2025-10-31（开发版）
...
- [IM SDK] 调用[删除服务端会话 API](conversation_delete.html#单向删除服务端会话)，成功后会删除本地会话。之前版本调用该接口可设置删除会话的本地消息，不能删除本地会话。
...
- [IM SDK] 新增[从服务器拉取离线消息的开始和结束的事件回调](/v4/android/connection): `EMConnectionListener#onOfflineMessageSyncStart` 和 `EMConnectionListener#onOfflineMessageSyncFinish`。
...
2. 增加接口支持获取历史消息(消息漫游)（`此功能需要联系商务开通`）;
3. 新增 PC 与移动端互发消息和文件的功能；
4. 增加消息撤回的接口和回调（`此功能需要联系商务开通`）;
5. 支持华为新版推送功能(HMS)；
