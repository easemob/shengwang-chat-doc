...
- 创建和发送附件类型消息。SDK 将附件上传到IM 服务器，获取消息的基本信息以及服务器上附件文件的路径。
- 对于图片消息，IM 服务器会自动生成图片缩略图；对于视频消息，视频的首帧为缩略图。
...
发消息时，你可以将消息附件上传至你自己的服务器（而非IM 服务器），然后发送消息。这种情况下，需要在 SDK 初始化时将 `Connection` 类中的 `useOwnUploadFun` 参数设置为 `true`。例如，对于图片消息，上传附件后，调用 `sendPrivateUrlImg` 方法传入图片的 URL 发送图片消息。
...
:::tip 
即时通讯 IM 小程序 SDK v4.19.0 及以上版本新增了便捷获取上传地址的方法。通过以下方式，你可以直接获取文件上传的服务器基地址（URL）：
