...
群详情页面、好友列表页面、聊天页面、会话列表页面和好友详情页面的导航栏均使用 `EaseChatNavigationBar`。如果群详情页面的导航栏不满足需求，建议自定义导航栏，重载方法传入自定义的导航类。关于导航栏中的标题、头像、背景色、导航栏右侧按钮的显示图片和左侧的头像，详见 [自定义导航栏](chatuikit_custom_titlebar.html#自定义导航栏)。
