...
初始化时，你需要通过 `ChatOptions.withAppId` 创建包含 App ID 的初始化配置。
...
React Native SDK 1.18.0 支持通过 `ChatOptions.withAppId` 的 `enableAutoSyncContacts` 参数配置登录后是否自动从服务器同步好友列表和好友对象。该参数默认为 `false`；设置为 `true` 后，用户登录成功时，SDK 会同步好友数据并更新本地数据。
...
| :--- | :--- | :--- | :--- |
| [`withAppId`](#初始化-sdk) | `ChatOptions` | `ChatOptions` | 使用 App ID 创建初始化配置。 |
