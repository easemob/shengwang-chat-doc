...
流式消息由业务服务端生成并通过 [RESTful API](/document/server-side/message_stream_send_single.html) 发送，Flutter SDK 负责接收、排序、合并和回调，不提供客户端主动发送流式消息的能力。
...
- **当前分片内容**：当前回调中接收到的单个分片内容，可通过 `ChatStreamChunk.chunk` 获取。
...
- **消息 ID**：流式消息的唯一标识，用于标识整条流式消息。在 Flutter SDK 中，可通过 `ChatMessage.msgId` 获取。
...
流式消息由业务服务端生成，并通过即时通讯 IM 服务端按分片方式下发至客户端 SDK。Flutter SDK 在分片到达时触发消息回调，并根据分片顺序自动合并消息内容。你可结合当前分片信息、累计合并内容和传输状态实时更新界面，直到整条消息完成或异常结束。
...
void handleStreamChunk(ChatMessage msg) {
  ...
  final ChatStreamChunk? chunk = msg.streamChunk;
  ...
  // 当前传输状态
  final ChatStreamStatus status = chunk.status;
  ...
  final String? customType = chunk.customType;
...
通过 `msg.streamChunk` 获取 `StreamChunk` 对象。该对象表示当前收到的分片信息。

| 属性 | 类型 | 说明 |
| :--- | :--- | :--- |
| `chunk` | String | 当前分片的文本内容。 |
| `status` | ChatStreamStatus | 当前分片对应的流式消息传输状态。 |
...
:::tip
- `chunk` 为当前分片的内容。
- `ChatTextMessageBody.content` 为从首个分片到当前分片的累计合并内容。
...
`ChatTextMessageBody.content` 表示从首个分片到当前分片的累计合并内容。
...
流式消息支持 `attributes` 扩展字段，但仅首个分片中的 `attributes` 会作为最终持久化结果保存到本地数据库。后续分片中的扩展字段为服务器透传，不应作为最终消息扩展信息依赖。

:::tip
`attributes` 使用建议如下：
- 与整条消息强相关的扩展信息放在首个分片中传递。
- 不要依赖后续分片动态修改 `attributes` 来驱动最终业务逻辑。
:::

### 传输状态与错误处理

你可以通过 `ChatStreamChunk.status` 获取当前分片对应的流式消息传输状态。

`ChatStreamStatus` 枚举说明如下：
...
| [消息搜索](message_search_local.html) | 支持 | 在本地或会话中搜索消息。 |
| [会话未读数](conversation_unread.html) | 支持 | 将消息计入会话未读数。 |
| 会话最后一条消息 | 支持 | 作为会话最后一条消息展示。 |
| [离线推送](/document/flutter/push/push_overview.html) | 支持 | 用户离线时进行消息推送提醒。 |
| [消息翻译](/value-added/translation/message_translation_flutter.html) | 支持 | 对消息内容进行翻译。 |
...
不支持。流式消息 [仅支持通过服务端 RESTful API 发送](/document/server-side/message_stream_send_single.html)，Flutter SDK 只负责接收。

#### 2. `chunk` 和 `content` 有何区别？

- `chunk`：当前分片内容。
- `content`：从首个分片到当前分片的累计合并内容。

通常 UI 展示应以 `content` 为准。
...
#### 4. 为什么后续分片 `attributes` 不生效？

因为最终持久化仅以首个分片中的 `attributes` 为准。后续分片不应用于更新最终消息扩展字段。
