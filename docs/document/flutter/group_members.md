...
| 入群方式                   | 公开群       | 私有群          |
| :------------------------- | :------------------ | :------------------------------------ |
| 用户申请入群       | 支持 <br/>任何用户均可申请入群，是否需要群主和群管理员审批，取决于群组类型 [ChatGroupStyle](https://doc.easemob.com/apidoc/flutter/im_flutter_sdk/ChatGroupStyle.html) 的设置。 | 不支持                                                                                             |
| 群成员邀请用户入群 | 支持 <br/>只能由群主和管理员邀请。    | 支持 <br/>除了群主和群管理员，群成员是否也能邀请其他用户进群取决于群组类型 [ChatGroupStyle](https://doc.easemob.com/apidoc/flutter/im_flutter_sdk/ChatGroupStyle.html) 的设置。 |

#### 用户申请入群

只有公开群支持用户申请入群，私有群不支持。用户可获取公开群列表，选择相应的群组 ID，然后调用相应方法加入该群组。

任何用户均可申请入群，是否需要群主和群管理员审批，取决于群组类型 [ChatGroupStyle](https://doc.easemob.com/apidoc/flutter/im_flutter_sdk/ChatGroupStyle.html) 的设置：
...
```dart
var groups = await ChatClient.getInstance.groupManager.fetchPublicGroupsFromServer(pageSize: 10, cursor: null);
List<ChatGroupInfo> groupList = groups.data;
String? cursor = groups.cursor;
```

2. 调用 `joinPublicGroup` 或 `requestToJoinPublicGroup` 方法传入群组 ID，申请加入对应群组。

   - 调用 `joinGroup` 方法加入无需群主或管理员审批的公开群，即 `ChatGroupStyle` 设置为 `PublicOpenJoin`。申请人不会收到任何回调，其他群成员会收到 `ChatGroupEventHandler#onMemberJoinedFromGroup` 和`ChatGroupEventHandler#onMembersJoinedFromGroup` 回调。
   ...
   - 调用 `requestToJoinPublicGroup` 方法加入需要群主或管理员审批的公开群，即 `ChatGroupStyle` 设置为 `PublicJoinNeedApproval`。示例代码如下：

   ```dart
   await ChatClient.getInstance.groupManager.requestToJoinPublicGroup(groupId, reason: "your reason");
   ```

   群主或群管理员收到 `ChatGroupEventHandler#onRequestToJoinReceivedFromGroup` 回调：

   - 若同意加入群组，需要调用 `acceptJoinApplication` 方法。

   申请人会收到 `ChatGroupEventHandler#onRequestToJoinAcceptedFromGroup` 回调，其他群成员会收到 `ChatGroupEventHandler#onMemberJoinedFromGroup` 和`ChatGroupEventHandler#onMembersJoinedFromGroup` 回调。

   示例代码如下：

   ```dart
   ChatClient.getInstance.groupManager.acceptJoinApplication(groupId, username);
   ```

   - 若群主或群管理员拒绝申请人入群，需要调用 `declineJoinApplication` 方法。申请人会收到 `ChatGroupEventHandler#onRequestToJoinDeclinedFromGroup` 回调。

   示例代码如下：

   ```dart
   ChatClient.getInstance.groupManager.declineJoinApplication(groupId, username, reason: "your reason");
   ```
...
1. 群成员邀请用户入群。
   ...
   ```dart
   ChatClient.getInstance.groupManager.addMembers(groupId, members);
   ```

   - 普通成员邀请人入群，需要调用 `inviterUser` 方法：

   对于私有群，`ChatGroupStyle` 设置为 `PrivateMemberCanInvite` 时，所有群成员均可以邀请人进群。

   ```dart
   ChatClient.getInstance.groupManager.inviterUser(groupId, members, reason: "your reason");
   ```
...
2. 受邀用户自动进群或确认是否加入群组：
   ...
   ```dart
   ChatClient.getInstance.groupManager.declineInvitation(groupId: groupId, inviter: inviter, reason: "your reason");
   ```
...
群成员可以调用 `leaveGroup` 方法退出群组。其他成员收到 `ChatGroupEventHandler#onMembersExitedFromGroup` 回调。
...
仅群主和群管理员可以调用 `ChatGroupManager#removeMembers` 方法将单个或多个成员移出群组。被踢出群组后，被踢成员将会收到群组事件回调 `ChatGroupEventHandler#onUserRemovedFromGroup`，其他成员将会收到回调 `ChatGroupEventHandler#onMembersExitedFromGroup`。被移出群组后，用户还可以再次加入群组。
...
仅群主和群管理员可以调用 `ChatGroupManager#blockMembers` 方法将指定成员添加至黑名单。被加入黑名单后，该成员收到 `ChatGroupEventHandler#onUserRemovedFromGroup` 事件。默认情况下，其他群成员不会收到事件通知。如需该事件，请联系商务开通。
