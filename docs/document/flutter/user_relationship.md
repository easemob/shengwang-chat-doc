...
- `onContactInfoUpdate`：原生 SDK 检测到好友信息更新时触发，返回更新后的 `ChatContact`，但不标识具体变更字段。类型为 `void Function(ChatContact contact)?`。昵称、头像等用户属性更新应另行监听 `ChatUserInfoEventHandler.onUserInfoUpdate`。
...
| [`withAppId`](#登录后自动同步好友列表) | `ChatOptions` | `ChatOptions` | 创建 SDK 配置，并通过 `enableAutoSyncContacts` 开启登录后自动同步好友数据。 |
