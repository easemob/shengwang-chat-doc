...
你已拥有有效的即时通讯 IM 开发者账号和 App ID，详见声网控制台 [注册账号](https://doc.shengwang.cn/doc/console/general/quickstart#注册账号) 和 [获取项目的 App ID](/product/enable_im.html#_3-获取-app-id) 说明。

## 初始化 SDK

使用 `ChatOptions.withAppId` 创建包含 App ID 的初始化配置，然后调用 `ChatClient.getInstance.init` 初始化 SDK。

```dart
final ChatOptions options = ChatOptions.withAppId(appId);
...
```dart
final ChatOptions options = ChatOptions.withAppId(
    appId,
...
下表列出了 `ChatOptions.withAppId` 的部分常用参数。全部参数请参见 [`ChatOptions` API 参考](https://doc.easemob.com/apidoc/flutter/im_flutter_sdk/ChatOptions-class.html)。
...
Flutter SDK 4.22.0 可通过 `ChatOptions.withAppId` 的 `enableAutoSyncContacts` 参数设置登录后是否自动同步好友列表和好友对象。该参数默认为 `false`；如需开启，必须在初始化 SDK 时设置为 `true`。好友对象包含用户 ID、备注和添加时间等信息，不代表已获取所有好友的昵称和头像。
...
| API 名称 | 所属模块/类 | 说明 |
| :--- | :--- | :--- |
| [`withAppId`](#初始化-sdk) | `ChatOptions` | 使用 App ID 创建初始化配置。 |
