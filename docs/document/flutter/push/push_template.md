...
- `default`：默认情况下，推送标题为 **您有一条新消息**，推送内容为 **请点击查看**。若调用了 `updatePushDisplayStyle` 方法将 `DisplayStyle` 设置为 `Simple`，则默认推送模板为 `default`。
 - `detail`：默认情况下，推送标题为 **您有一条新消息**，推送内容为消息内容。若调用了 `updatePushDisplayStyle` 方法将 `DisplayStyle` 设置为 `Summary`，则默认推送模板为 `detail`。
