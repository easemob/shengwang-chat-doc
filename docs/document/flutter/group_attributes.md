...
仅群主和群管理员可以调用 `ChatGroupManager#updateGroupName` 方法设置和修改群组名称，群名称的长度限制为 128 个字符, 其他成员会收到 `ChatGroupEventHandler#onSpecificationDidUpdate` 回调。
...
仅群主和群管理员可以调用 `ChatGroupManager#updateGroupDesc` 方法设置和修改群组描述，群描述的长度限制为 512 个字符, 其他成员会收到 `ChatGroupEventHandler#onSpecificationDidUpdate` 回调。
