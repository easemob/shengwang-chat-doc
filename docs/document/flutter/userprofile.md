...
如需 SDK 在登录成功后自动同步好友列表及好友信息，需在初始化前设置 `ChatOptions.withAppId` 的 `enableAutoSyncContacts: true`。同步完成后，可通过 `ChatContactManager#getAllContacts`、`getContact` 或 `getAllContactIds` 读取本地好友数据。详见 [登录后自动同步好友列表](user_relationship.html#登录后自动同步好友列表)。
