...
应用客户端成功连接到IM 服务器后，才能使用即时通讯 SDK 的收发消息等功能。

你调用 `loginWithToken` 方法登录后，客户端 SDK 会自动连接IM 服务器。关于登录详情，请参见[登录文档](login.html)。
...
- 开启多设备服务后，用户在其他设备上通过调用 API 或者声网控制台将当前设备登录的 ID 强制退出登录（错误码 217）。