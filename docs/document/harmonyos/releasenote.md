...
- 修复应用从后台切回前台且连接已断开时，未正确触发网络变化通知导致重连检查异常的问题。
- 修复离线消息同步过程中，并发访问同步队列可能引发的崩溃问题。
- 修复离线消息同步完成回调在持有同步队列锁时触发，可能导致锁等待和卡顿的问题。
- 修复带翻译语言的消息发送失败的问题。
- 优化文件附件分片上传及 HTTP 错误处理逻辑，提升附件上传稳定性。
- 修复群共享文件下载失败后临时文件未清理的问题。
- 修复特定场景下群组或聊天室成员数量统计异常的问题。
- 修复批量获取群成员信息时未触发用户信息更新回调的问题。
- 修复添加不存在的用户为好友时返回错误码不准确的问题。
- 修复获取用户属性失败时错误码不准确的问题。
- 修复获取好友失败后重试请求地址不一致的问题。

## v1.12.1 Dev 2026-05-06（开发版）

#### 优化

将网络变化监听的注册时机调整至登录流程之后，避免在未登录阶段提前注册。

#### 修复

移除 TCP 连接成功日志中的客户端本机 IP 地址，以降低隐私泄露风险。
...
## 版本 V1.9.2 Dev 2026-7-30（开发版）

### 优化

- 优化异步日志写入：限制日志任务队列积压，并减少日志路径上的字符串拷贝，降低高日志量场景下的内存与性能压力。
- 应用进入后台时抑制传输层冗余日志，降低后台日志写入压力。
- 登录时自动将 `userId` 中的大写字母转换为小写，避免因大小写不一致导致的用户匹配问题。

### 修复

- 修复日志敏感字段脱敏在字段值未变化时可能停止继续脱敏的问题。
- 修复强制登出与自动登录凭证保存及清理并发时可能出现竞态，导致自动登录状态判断异常的问题。

## v1.9.1

#### 修复

- 修复前台恢复时，心跳发送、连接断开及重连逻辑可能阻塞主线程的问题；
- 修复应用切至前台后，在 SDK 已登录、网络可用但连接异常断开时，未及时通知 SDK 网络状态变化导致重连延迟的问题；
- 修复离线消息同步队列因并发访问导致的潜在崩溃问题；
- 修复会话排序流程中可能触发的崩溃异常；
- 修复查询群组或聊天室成员是否在禁言列表或白名单中时，异常场景下逻辑处理不完善的问题。
