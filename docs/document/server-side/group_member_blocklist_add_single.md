...
| `usernames`  | String    |  是       | 要添加至群组黑名单的用户 ID。 |
...
curl -X POST 'https://XXXX/app-id/{app_id}/chatgroups/66XXXX85/blocks/users/user1'   \
...
| - `action`  | String | 执行操作。在该响应中，该字段的值为 `add_blocks`，表示将成员添加至群组黑名单。 |
