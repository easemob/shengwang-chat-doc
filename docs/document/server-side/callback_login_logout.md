...
用户成功登录 Web 端时，IM 服务器可以实时感知到状态变成在线：
...
用户成功登录小程序端时，IM 服务器可以实时感知到状态变成在线：
...
用户登录、登出或被踢下线时，即时通讯 IM 服务器会向你的 App Server 发送状态变更回调。三种回调的请求字段结构相同，区别仅在于 `reason` 和 `status` 字段的值。
