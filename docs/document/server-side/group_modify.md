...
curl -X PUT 'https://XXXX/app-id/{app_id}/chatgroups/6XXXX7'   \
...
| 400     | invalid_parameter                  | "some of [groupid] are not valid fields"  | 修改的群组信息时，传入的参数不支持，例如修改 `groupid`。| 请存入存在的群组 ID。  |
