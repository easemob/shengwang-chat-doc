...
- 该操作仅断开指定设备的连接，用户在其他设备上的登录状态不受影响。被强制下线的设备在断线期间收到的消息，会作为该设备的离线消息由服务端保留，该设备重新登录后可同步到这些消息；同时在线的其他设备不受影响，仍会实时收到消息。离线消息默认最长存储 7 天，如果该设备 7 天内都没有重新上线，服务端将丢弃过期的消息。
