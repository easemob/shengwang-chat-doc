...
curl -X POST 'https://XXXX/app-id/{app_id}/chatgroups'  \
...
| 403     | exceed_limit | user XX has joined too many groups! | 用户加入的群组数量达到上限。 | 退出不用的群组或在 [声网控制台](https://console.shengwang.cn/overview) 上的 **套餐包 > 预付费套餐包 > 即时通讯 IM** 上调单个用户可加入群组数上限。 |
