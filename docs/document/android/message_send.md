...
默认情况下，调用 `EMChatManager#sendMessage` 后，SDK 会自动将本地附件上传至IM 服务器；接收方也可由 SDK 自动下载附件。你可以通过 `EMOptions#setAutoTransferMessageAttachments` 控制附件是否由 SDK 自动传输。消息附件大小和存储限制，详见 [消息附件限制说明](/product/limitation.html#消息存储)。
...
对于聊天室消息，即时通讯 IM 支持高、普通和低三种消息优先级。你可以通过 `EMMessage#setPriority` 设置单条聊天室消息的优先级。
