---
title: 即时通讯 IM 主要功能
show_online_push: false
---

即时通讯 IM 支持消息管理、用户管理功能和离线推送功能。
...
使用多端登录前，你需要在 [声网控制台](https://console.shengwang.cn/overview) 的 **即时通讯 IM** > **功能配置** > **用户与登陆** 页面开启 **多端多设备** 功能。开通该功能后可以设置各端设备的数量。
...
| 消息撤回       | 撤回发送成功的消息。默认可撤回发出 2 分钟内的消息。你可以在联系声网设置消息撤回时长，该时长不超过 7 天。 |
| 消息回执  | 消息送达回执和消息已读回执。    |
| 编辑消息  | 编辑发送成功的消息。    |
| 表情回复  | 对消息添加、删除表情。   |
...
| IM 用户设备日志 | 查询用户设备端的日志，或将设备端的日志上传到 IM 日志服务器。  |
...
<HideSection :show="$frontmatter.show_online_push">
