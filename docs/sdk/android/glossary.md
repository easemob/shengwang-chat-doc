...
即时通讯 IM SDK 是提供的用于实现即时通讯，比如：单聊、群聊、聊天室的 SDK。

### 单群聊 UIKit

单群聊 UIKit 是即时通讯 IM SDK 的一个开源 UI 组件，提供应用内聊天的常用页面和 UI 组件，帮助开发者快速构建应用的 UI。 

### 声网控制台

[声网控制台](https://console.shengwang.cn/overview)是声网提供给开发者管理声网各项服务的工具。

## 日活用户

日活用户（DAU），指日活跃用户数，即一个自然日内与声网服务器进行业务行为交互（包含但不局限于登录、收发消息）的用户总数。
...
用户 ID 即用户名，是 App 内用户的唯一标识，代码中出现的“username”、“userId”、“user”参数指定了用户 ID。用户 ID 不同于即时通讯系统服务器为用户创建的 UUID。

### UUID

即时通讯服务器为 App 内用户创建的唯一 ID，不同于用户 ID。
...
离线是指用户成功登出即时通讯 IM 系统或与即时通讯 IM 系统断开连接后的状态。用户登出即时通讯 IM 系统之后，无法发送和接收消息，在下次登录后可以接收到离线消息。
...
即时通讯 IM RESTful 平台提供一个多租户用户体系，以集合（Collection）的形式的形式管理资源，一个集合包括数据库、企业、应用、用户、群组、消息和文件等。
